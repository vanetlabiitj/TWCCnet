...
class STGeoModule(nn.Module):
    def __init__(self,grid_in_channel,num_of_gru_layers,seq_len,
        ...
        self.grid_gru = nn.GRU(input_size=grid_in_channel,hidden_size= int(gru_hidden_size/2),num_layers= num_of_gru_layers,batch_first=True,bidirectional =True)
...
class STSemModule(nn.Module):
    def __init__(self,num_of_graph_feature,nums_of_graph_filters,
        ...
        self.road_conv = nn.Sequential(
            nn.Conv2d(in_channels=1,out_channels=1,kernel_size=1),
            #nn.ReLU(),
        )

        self.risk_conv = nn.Sequential(
            nn.Conv2d(in_channels=1,out_channels=1,kernel_size=1),
            #nn.ReLU(),
        )

        self.poi_conv = nn.Sequential(
            nn.Conv2d(in_channels=1,out_channels=1,kernel_size=1),
            #nn.ReLU(),
        )

        self.graph_gru = nn.GRU(input_size=num_of_filter,hidden_size=int(gru_hidden_size/2),num_layers= num_of_gru_layers,batch_first=True,bidirectional =True)
    ...
    def forward(self,graph_feature,road_adj,risk_adj,poi_adj,
        ...
        road_graph_output = road_graph_output.view(batch_size,1,-1,N)
        road_graph_output = self.road_conv(road_graph_output)
        road_graph_output = road_graph_output.view(batch_size,-1,N).view(-1,N,batch_size)
        graph_output = road_graph_output
        ...
        risk_graph_output = risk_graph_output.view(batch_size,1,-1,N)
        risk_graph_output = self.risk_conv(risk_graph_output)
        risk_graph_output = risk_graph_output.view(batch_size,-1,N).view(-1,N,batch_size)
        graph_output = graph_output + risk_graph_output
        ...
        if poi_adj is not None:
            ...
            poi_graph_output = poi_graph_output.view(batch_size,1,-1,N)
