...
class STSemModule(nn.Module):
    ...
    def forward(self,graph_feature,road_adj,risk_adj,poi_adj,
        ...
        # road_graph_output = road_graph_output.view(batch_size,T,N,-1)\
        #                             .permute(0,2,1,3)\
        #                             .contiguous()\
        ...
        # risk_graph_output = risk_graph_output.view(batch_size,T,N,-1)\
        #                             .permute(0,2,1,3)\
        #                             .contiguous()\
        #                             .view(batch_size*N,T,-1)
        ...
        if poi_adj is not None:
            ...
            # poi_graph_output = poi_graph_output.view(batch_size,T,N,-1)\
            #                         .permute(0,2,1,3)\
            #                         .contiguous()\
            #                         .view(batch_size*N,T,-1)
