...
from model.TWCCNet import TWCCNet
...
def training(net,
    ...
    for epoch in range(1,training_epoch+1):
        ...
        print("road: ",net.st_sem_module.road_conv[0].weight)
        print("risk: ",net.st_sem_module.risk_conv[0].weight)
        print("poi: ",net.st_sem_module.poi_conv[0].weight)
...
def main(config):
    ...
    TWCCNet_Model = TWCCNet(train_data_shape[2],num_of_gru_layers,seq_len,pre_len,
                    gru_hidden_size,time_shape[1],graph_feature_shape[2],
                    nums_of_filter,north_south_map,west_east_map)
    #multi gpu
    if torch.cuda.device_count() > 1:
        print("Let's use", torch.cuda.device_count(), "GPUs!",flush=True)
        TWCCNet_Model = nn.DataParallel(TWCCNet_Model)
    TWCCNet_Model.to(device)
    print(TWCCNet_Model)

    num_of_parameters = 0
    for name,parameters in TWCCNet_Model.named_parameters():
        num_of_parameters += np.prod(parameters.shape)
    print("Number of Parameters: {}".format(num_of_parameters), flush=True)


    trainer = optim.Adam(TWCCNet_Model.parameters(), lr=learning_rate)
